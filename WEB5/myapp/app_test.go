...
func TestCreateUserInfo(t *testing.T) {
	...
	resp, err := http.Post(ts.URL+"/users", "application/json", strings.NewReader(`{"first_name":"gildong", "last_name":"Yun", "email":"gildong@naver.com"}`))
	assert.NoError(err)
...
func TestUpdateUser(t *testing.T) {
	...
	updateStr := fmt.Sprintf(`{"id":%d, "first_name":"jason", "last_name":"YUN"}`, user.ID)
	...
	assert.Equal("YUN", updateUser.LastName)
...
func TestUsers_WithUsers(t *testing.T) {
	...
	resp, err := http.Post(ts.URL+"/users", "application/json",
		strings.NewReader(`{"first_name":"gildong1", "last_name":"Yun1", "email":"gildong@naver.com1"}`))
	assert.NoError(err)
	assert.Equal(http.StatusCreated, resp.StatusCode)

	resp, err = http.Post(ts.URL+"/users", "application/json",
		strings.NewReader(`{"first_name":"gildong2", "last_name":"Yun2", "email":"gildong@naver.com2"}`))
	assert.NoError(err)
	assert.Equal(http.StatusCreated, resp.StatusCode)
